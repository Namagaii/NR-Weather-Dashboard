const REQUESTROOT = "http://api.openweathermap.org/data/2.5/weather?appid=f310e487a963435a12a4d57a94cd2dff"; //root
let weatherData;
...
function DisplayTodaysData(p_weatherData){
    ...
    _todaysDisplay.children("p").eq(3).children("mark").attr("id", GetUVColor(p_weatherData.uvi));
}

function DisplayFiveDayData(p_weatherData, p_index){
    let _todaysDisplay = $("#Five-Day-Forecast").children("#Day-"+p_index);
...
function UpdateSearchHistory(query){
    ...
    if (localStorage.getItem(SEARCHHISTORYKEY)){
        _wasPopulated = true;
        let _searchHistory = JSON.parse(localStorage.getItem(SEARCHHISTORYKEY));
        for (let i = 0; i < _searchHistory.numResults; i++){
